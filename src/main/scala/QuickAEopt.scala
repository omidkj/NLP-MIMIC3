...
class QuickAEopt extends org.apache.uima.fit.component.JCasAnnotator_ImplBase{
  override def process(aJCAS:JCas): Unit = {
    ...
    val combin = diseaseOrDisorders_array.map { d =>
      JCasUtil.select(d.getOntologyConceptArr(), classOf[UmlsConcept])
    }
    val con_coll = combin.map(con => con.toArray(new Array[UmlsConcept](0)))
    val cui_coll = con_coll.flatMap(cty => cty.map(elem => elem.getCui))
    val cui_str_list = cui_coll.map(cui => cui + ",")
    val cui_str = cui_str_list.mkString
    csvWriter2.writeNext(cui_str)
